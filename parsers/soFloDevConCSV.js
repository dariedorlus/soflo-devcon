  ...
  if (row[0] && row[0].indexOf('min') == -1) {
    ...
    if (row[0].indexOf('-') == -1) {
      ...
      for (let x = colSessionsStart; x < row.length; x++) {
        ...
        let sessionTime = row[0]
          .substr(0, row[0].indexOf(' - '))
          .replace(' ', '')
          .replace('AM', '')
          .replace('PM', '');
        ...
        firstpartOfTime =
          Number(firstpartOfTime) <= 7
            ? Number(firstpartOfTime) + 12
            : '0' + firstpartOfTime;
        ...
        if (!TimeSlots.find((x) => x?.time == Sessions[currentX].startTime)) {
          TimeSlots.push({
            time: Sessions[currentX].startTime,
            order: Number(firstpartOfTime),
            conferenceId: conferenceId,
          });
        }
